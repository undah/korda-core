    ...
    <div style={{ marginBottom: "1.25rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "0.4rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: "0.75rem" }}>
          <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.75rem", color: "rgba(221,232,237,0.4)" }}>{from}{unit} → {to}{unit}</span>
...
export default function TrackerPhotos() {
  ...
  const doUpload = async () => {
    ...
    try {
      await uploadPhoto.mutateAsync({
        file, angle: form.angle, log_date: form.log_date,
  ...
  return (
    <div>
      ...
      {showPhotoConflict && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(7,9,11,0.88)", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(6px)" }}>
          <div style={{ background: "#0c1217", border: "1px solid rgba(90,180,212,0.15)", borderTop: "1px solid rgba(90,180,212,0.5)", padding: "2.5rem", maxWidth: 420, width: "90%" }}>
            <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.58rem", letterSpacing: "0.22em", textTransform: "uppercase", color: "rgba(90,180,212,0.5)", marginBottom: "1rem" }}>
              // conflict detected
            </p>
            ...
            <div style={{ display: "flex", gap: "0.75rem" }}>
              <button className="kt-btn kt-btn-blue" onClick={doUpload} disabled={uploadPhoto.isPending} style={{ flex: 1 }}>
                {uploadPhoto.isPending ? "Uploading..." : "Replace →"}
              </button>
              <button className="kt-btn kt-btn-outline" onClick={() => setShowPhotoConflict(false)} style={{ flex: 1 }}>
                Cancel
              </button>
            </div>
          </div>
      ...
      {/* ── LIGHTBOX ── */}
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(7,9,11,0.95)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          <div onClick={e => e.stopPropagation()} style={{ maxWidth: "90vw", maxHeight: "90vh", position: "relative" }}>
            <img src={lightbox.url} alt={lightbox.angle} style={{ maxWidth: "80vw", maxHeight: "80vh", objectFit: "contain", display: "block" }} />
            <div style={{ marginTop: "1rem", textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: "1.5rem" }}>
              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.72rem", color: "rgba(221,232,237,0.4)", textTransform: "capitalize" }}>
            ...
            <button onClick={() => setLightbox(null)} style={{ position: "absolute", top: -40, right: 0, background: "none", border: "none", color: "rgba(221,232,237,0.4)", fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.8rem", cursor: "pointer" }}>
              close ×
            </button>
          </div>
        </div>
      ...
      <div style={{ display: "flex", gap: 2, marginBottom: "2rem" }}>
        {([["timeline", "Timeline"], ["compare", "Compare"]] as [Tab, string][]).map(([t, label]) => (
          <button key={t} onClick={() => t === "compare" ? handleTabCompare() : setTab(t)}
            style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.7rem", letterSpacing: "0.12em", textTransform: "uppercase", padding: "0.6rem 1.5rem", cursor: "pointer", border: "none", background: tab === t ? "#0c1217" : "transparent", color: tab === t ? "#5ab4d4" : "rgba(221,232,237,0.3)", borderBottom: tab === t ? "1px solid rgba(90,180,212,0.5)" : "1px solid rgba(90,180,212,0.08)", transition: "all 0.15s" }}>
            {label}
          </button>
        ))}
      ...
      {tab === "timeline" && (
        <>
          <div className="kt-card" style={{ marginBottom: "2rem" }}>
            ...
            <div className="kt-grid-3" style={{ gap: "1rem", marginBottom: "1.5rem" }}>
              <div>
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  {ANGLES.map(a => (
                    <button key={a} onClick={() => setForm(f => ({ ...f, angle: a }))}
                      style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.7rem", letterSpacing: "0.08em", textTransform: "capitalize", padding: "0.5rem 0.9rem", cursor: "pointer", border: "1px solid", background: form.angle === a ? "rgba(90,180,212,0.1)" : "transparent", borderColor: form.angle === a ? "rgba(90,180,212,0.5)" : "rgba(221,232,237,0.1)", color: form.angle === a ? "#5ab4d4" : "rgba(221,232,237,0.3)", transition: "all 0.15s" }}>
                      {a}
                    </button>
                  ))}
                </div>
            ...
            <div onClick={() => fileRef.current?.click()}
              style={{ border: "1px dashed rgba(90,180,212,0.2)", padding: "2.5rem", textAlign: "center", cursor: "pointer", marginBottom: "1.5rem", transition: "border-color 0.2s", background: preview ? "transparent" : "rgba(90,180,212,0.02)" }}
              ...
              {preview ? (
                <img src={preview} alt="preview" style={{ maxHeight: 240, maxWidth: "100%", objectFit: "contain", display: "block", margin: "0 auto" }} />
              ) : (
                <>
                  <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.75rem", color: "rgba(90,180,212,0.5)", marginBottom: "0.5rem" }}>click to select photo</p>
                  <p style={{ fontSize: "0.75rem", color: "rgba(221,232,237,0.2)" }}>JPG, PNG, WEBP supported</p>
                </>
              )}
            </div>
            <input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={handleFile} />
            <button className="kt-btn kt-btn-blue" onClick={handleUpload} disabled={!file || uploadPhoto.isPending}>
          ...
          {isLoading ? (
            ...
            <p style={{ color: "rgba(221,232,237,0.3)", fontSize: "0.85rem" }}>No photos yet. Upload your first progress photo.</p>
          ) : (
                ...
                <div key={date}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "0.75rem" }}>
                    {ANGLES.map(angle => {
                      ...
                      return (
                        <div key={angle} style={{ position: "relative", aspectRatio: "3/4", background: "#0c1217", border: "1px solid rgba(90,180,212,0.08)", overflow: "hidden" }}>
                          {photo ? (
                            <>
                              <img src={photo.url} alt={angle} onClick={() => setLightbox(photo)}
                                style={{ width: "100%", height: "100%", objectFit: "cover", cursor: "pointer", display: "block", transition: "opacity 0.2s" }}
                                onMouseEnter={e => (e.currentTarget.style.opacity = "0.8")}
                                onMouseLeave={e => (e.currentTarget.style.opacity = "1")} />
                              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0.5rem", background: "linear-gradient(transparent,rgba(7,9,11,0.8))", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.6rem", letterSpacing: "0.1em", textTransform: "capitalize", color: "rgba(221,232,237,0.5)" }}>{angle}</span>
                                <button onClick={e => { e.stopPropagation(); setConfirmDeleteId(photo.id); }}
                                  style={{ background: "none", border: "none", color: "rgba(212,112,90,0.5)", cursor: "pointer", fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.6rem", transition: "color 0.2s" }}
                                  onMouseEnter={e => (e.currentTarget.style.color = "rgba(212,112,90,0.9)")}
                                  onMouseLeave={e => (e.currentTarget.style.color = "rgba(212,112,90,0.5)")}>×</button>
                              </div>
                            </>
                          ) : (
                            <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "0.5rem" }}>
                              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.62rem", letterSpacing: "0.1em", textTransform: "capitalize", color: "rgba(221,232,237,0.15)" }}>{angle}</span>
                              <span style={{ fontSize: "0.7rem", color: "rgba(221,232,237,0.1)" }}>no photo</span>
                            </div>
                          )}
                        </div>
                      );
        ...
        <div>
          {dates.length < 2 ? (
            <div className="kt-card" style={{ textAlign: "center", padding: "4rem 2rem" }}>
              <p style={{ fontFamily: "'Playfair Display',serif", fontSize: "1.4rem", marginBottom: "1rem" }}>Not enough photos.</p>
              <p style={{ color: "rgba(221,232,237,0.4)", fontSize: "0.9rem" }}>You need photos from at least 2 different dates to compare.</p>
            </div>
          ) : (
            <>
              <div className="kt-card" style={{ marginBottom: "2rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem" }}>
                  <div>
                    <label className="kt-label">From (before)</label>
                  ...
                  <div>
                    <label className="kt-label">To (after)</label>
                    ...
                    <div style={{ display: "flex", gap: "0.5rem" }}>
                      {ANGLES.map(a => (
                        <button key={a} onClick={() => setCompareAngle(a)}
                          style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.7rem", letterSpacing: "0.08em", textTransform: "capitalize", padding: "0.5rem 0.9rem", cursor: "pointer", border: "1px solid", background: compareAngle === a ? "rgba(90,180,212,0.1)" : "transparent", borderColor: compareAngle === a ? "rgba(90,180,212,0.5)" : "rgba(221,232,237,0.1)", color: compareAngle === a ? "#5ab4d4" : "rgba(221,232,237,0.3)", transition: "all 0.15s" }}>
                          {a}
                        </button>
                      ))}
                    </div>
              ...
              {dateA && dateB ? (
                <>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, marginBottom: 2 }}>
                    {[{ date: dateA, photo: photoA, label: "Before", checkin: checkinA }, { date: dateB, photo: photoB, label: "After", checkin: checkinB }].map(({ date, photo, label, checkin }) => (
                      <div key={date} style={{ background: "#0c1217", padding: "1.5rem", borderTop: `1px solid ${label === "Before" ? "rgba(221,232,237,0.1)" : "rgba(90,180,212,0.4)"}` }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                          <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.62rem", letterSpacing: "0.2em", textTransform: "uppercase", color: label === "Before" ? "rgba(221,232,237,0.3)" : "#5ab4d4" }}>// {label}</p>
                          <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.65rem", color: "rgba(221,232,237,0.3)" }}>{date}</span>
                        </div>
                        {photo ? (
                          <div style={{ aspectRatio: "3/4", overflow: "hidden", marginBottom: "1rem", cursor: "pointer" }} onClick={() => setLightbox(photo)}>
                            <img src={photo.url} alt={label} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", transition: "opacity 0.2s" }}
                              onMouseEnter={e => (e.currentTarget.style.opacity = "0.85")}
                              onMouseLeave={e => (e.currentTarget.style.opacity = "1")} />
                          </div>
                        ) : (
                          <div style={{ aspectRatio: "3/4", background: "#0a0e12", border: "1px dashed rgba(90,180,212,0.1)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "1rem" }}>
                            <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.7rem", color: "rgba(221,232,237,0.15)" }}>no {compareAngle} photo</p>
                          </div>
                        )}
                        {checkin && (
                          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                            {checkin.weight   && <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem" }}><span style={{ color: "rgba(221,232,237,0.3)" }}>Weight</span><span style={{ fontFamily: "'IBM Plex Mono',monospace", color: label === "After" ? "#5ab4d4" : "#dde8ed" }}>{checkin.weight} kg</span></div>}
                            {checkin.waist    && <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem" }}><span style={{ color: "rgba(221,232,237,0.3)" }}>Waist</span><span style={{ fontFamily: "'IBM Plex Mono',monospace", color: "rgba(221,232,237,0.6)" }}>{checkin.waist} cm</span></div>}
                            {checkin.chest    && <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem" }}><span style={{ color: "rgba(221,232,237,0.3)" }}>Chest</span><span style={{ fontFamily: "'IBM Plex Mono',monospace", color: "rgba(221,232,237,0.6)" }}>{checkin.chest} cm</span></div>}
                            {checkin.body_fat && <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem" }}><span style={{ color: "rgba(221,232,237,0.3)" }}>Body fat</span><span style={{ fontFamily: "'IBM Plex Mono',monospace", color: "rgba(221,232,237,0.6)" }}>{checkin.body_fat}%</span></div>}
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  {(checkinA || checkinB) && (
                    <div className="kt-card">
                      ...
                      {checkinA?.weight && checkinB?.weight && (
                        <div style={{ marginTop: "1.5rem", paddingTop: "1.5rem", borderTop: "1px solid rgba(90,180,212,0.07)", display: "flex", gap: "2rem", flexWrap: "wrap" }}>
                          <div>
                            <p className="kt-card-label">Total change</p>
                            <p style={{ fontFamily: "'Playfair Display',serif", fontSize: "2rem", fontWeight: 400, color: checkinB.weight < checkinA.weight ? "#5ad4a0" : "#d4705a" }}>
                          ...
                          <div>
                            ...
                            <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "1rem", color: "#dde8ed" }}>
                ...
                <div className="kt-card" style={{ textAlign: "center", padding: "3rem" }}>
                  <p style={{ color: "rgba(221,232,237,0.3)", fontSize: "0.9rem" }}>Select two dates above to start comparing.</p>
                </div>
              )}
            </>
          )}
