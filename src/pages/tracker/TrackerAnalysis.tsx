...
export default function TrackerAnalysis() {
  ...
  // adherence (days with checkin / total days since first)
  ...
  return (
    <div>
      ...
      <div className="kt-grid-4" style={{ marginBottom: 2 }}>
        ...
        <div className="kt-card">
          <p className="kt-card-label">Projected arrival</p>
          <p className="kt-card-value" style={{ fontSize: "1.1rem" }}>
            {stats?.daysToGoal ? projectedDate(stats.daysToGoal) : "—"}
          </p>
          <p className="kt-card-sub">{stats?.daysToGoal ? `${stats.daysToGoal} days` : "set a goal"}</p>
        </div>
        <div className="kt-card">
          <p className="kt-card-label">Check-in adherence</p>
          ...
          <thead>
            <tr>
              {["Week","Avg weight","Change","Entries","Trend"].map(h => (
                <th key={h} style={{ textAlign: "left", padding: "0.5rem 0.75rem", fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.6rem", letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(221,232,237,0.25)", borderBottom: "1px solid rgba(90,180,212,0.07)" }}>
                  {h}
                </th>
              ))}
            </tr>
        ...
        <div className="kt-card" style={{ marginBottom: "1.5rem" }}>
          ...
          <div className="kt-grid-3">
            {[
              ...
              { label: "Waist delta", first: null, last: null, delta: waistDelta },
            ].map(m => {
              ...
              return (
                <div key={m.label} className="kt-card" style={{ border: "none", borderTop: "1px solid rgba(90,180,212,0.1)", background: "#0f151a" }}>
                  ...
                  {m.first && m.last ? (
                    <>
                      <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "1rem", color: "#dde8ed" }}>{m.first} → {m.last} cm</p>
                    ...
                    <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "1.2rem", color: delta < 0 ? "#5ad4a0" : "#d4705a" }}>
                      {delta > 0 ? "+" : ""}{delta} cm
                    </p>
                  ) : (
      ...
      <div className="kt-card">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          {generateInsights(stats, adherence, avgDeficit, sorted).map((insight, i) => (
            <div key={i} style={{ display: "flex", gap: "1rem", padding: "0.75rem", background: "#0a0e12", borderLeft: `2px solid ${insight.color}` }}>
              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: "0.7rem", color: insight.color, flexShrink: 0 }}>{insight.tag}</span>
              <p style={{ fontSize: "0.85rem", color: "rgba(221,232,237,0.6)", lineHeight: 1.6 }}>{insight.text}</p>
            </div>
          ))}
        </div>
      </div>
...
function generateInsights(
  ...
  else if (adherence < 50) insights.push({ tag: "consistency", color: "#d4705a", text: `You've only logged ${adherence}% of days. More frequent check-ins will give you a clearer trend and better projections.` });

  if (stats && stats.avgWeeklyLoss >= 0.5 && stats.avgWeeklyLoss <= 1) {
    insights.push({ tag: "rate", color: "#5ad4a0", text: `Your average weekly loss of ${stats.avgWeeklyLoss} kg is in the optimal range (0.5–1 kg/week) for fat loss while preserving muscle.` });
  } else if (stats && stats.avgWeeklyLoss > 1.2) {
    insights.push({ tag: "rate", color: "#d4b45a", text: `You're losing ${stats.avgWeeklyLoss} kg/week, which is faster than ideal. Consider increasing calories slightly to preserve muscle mass.` });
  }

  if (avgDeficit && avgDeficit > 800) {
    insights.push({ tag: "deficit", color: "#d4705a", text: `Your average deficit of ${avgDeficit} kcal is quite aggressive. This may cause muscle loss, fatigue, and rebound. Consider a more moderate deficit of 300–600 kcal.` });
  } else if (avgDeficit && avgDeficit >= 300 && avgDeficit <= 600) {
    insights.push({ tag: "deficit", color: "#5ad4a0", text: `A ${avgDeficit} kcal average daily deficit is sustainable and effective. You're in a good range.` });
  }
