...
const moodColor = (m: string) => ({ great: "#22C55E", good: "#5ab4d4", okay: "var(--kt-muted)", low: "#d4b45a", bad: "#EF4444" }[m] ?? "rgba(232,232,240,0.3)");
  ...
  return (
    <div>
          ...
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            {MOODS.map(m => (
              <button key={m} onClick={() => setForm(f => ({ ...f, mood: m }))}
                style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "var(--kt-fs-xs)", textTransform: "capitalize", padding: "0.4rem 1rem", cursor: "pointer", border: "1px solid", background: form.mood === m ? moodColor(m) + "22" : "transparent", borderColor: form.mood === m ? moodColor(m) : "var(--kt-border)", color: form.mood === m ? moodColor(m) : "var(--kt-dim)", transition: "all 0.15s" }}>
                {m}
              </button>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: "0.5rem" }}>
            {ENERGYS.map(e => (
              <button key={e} onClick={() => setForm(f => ({ ...f, energy: e }))}
                style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "var(--kt-fs-xs)", textTransform: "capitalize", padding: "0.4rem 1rem", cursor: "pointer", border: "1px solid", background: form.energy === e ? "var(--kt-accent-bg)" : "transparent", borderColor: form.energy === e ? "var(--kt-accent)" : "var(--kt-border)", color: form.energy === e ? C.accent : C.dim, transition: "all 0.15s" }}>
                {e}
              </button>
            ))}
          </div>
      ...
      <div>
        ...
        {isLoading ? (
          <p style={{ color: C.dim, fontFamily: "'DM Sans',sans-serif", fontSize: "var(--kt-fs-sm)" }}>Loading...</p>
        ) : sorted.length === 0 ? (
          <p style={{ color: C.dim, fontSize: "var(--kt-fs-md)" }}>No journal entries yet.</p>
        ) : (
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          <button onClick={(e) => onDelete(entry.id, e)}
            style={{ background: "none", border: "none", color: "rgba(239,68,68,0.35)", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: "var(--kt-fs-2xs)", transition: "color 0.2s" }}
            onMouseEnter={e => (e.currentTarget.style.color = "rgba(239,68,68,0.8)")}
            onMouseLeave={e => (e.currentTarget.style.color = "rgba(239,68,68,0.35)")}>
            delete
          </button>
          <span style={{ color: C.accent, opacity: 0.4, fontSize: "var(--kt-fs-sm)", display: "inline-block", transform: open ? "rotate(90deg)" : "none", transition: "transform 0.2s" }}>→</span>
        </div>
      </div>
      {open && (
        <div style={{ marginTop: "1rem", paddingTop: "1rem", borderTop: `1px solid ${C.border}` }}>
          {entry.notes && <p style={{ fontSize: "var(--kt-fs-md)", color: C.muted, lineHeight: 1.7, marginBottom: "1rem" }}>{entry.notes}</p>}
          <div className="kt-grid-2" style={{ gap: "1rem" }}>
            {entry.wins && <div><p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "var(--kt-fs-2xs)", color: C.green, textTransform: "uppercase", marginBottom: "0.4rem" }}>Wins</p><p style={{ fontSize: "var(--kt-fs-sm)", color: C.muted, lineHeight: 1.6 }}>{entry.wins}</p></div>}
            {entry.struggles && <div><p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "var(--kt-fs-2xs)", color: C.red, textTransform: "uppercase", marginBottom: "0.4rem" }}>Struggles</p><p style={{ fontSize: "var(--kt-fs-sm)", color: C.muted, lineHeight: 1.6 }}>{entry.struggles}</p></div>}
          </div>
        </div>
      )}
    </div>
