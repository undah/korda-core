...
import { bulkInsertConceptEntries } from '../lib/trainingData';
...
function parseCSV(text: string): ParsedRow[] {
  ...
  return lines.slice(1).map((line, i) => {
    ...
    const concept      = raw['concept'] ?? raw['concept_name'] ?? raw['name'] ?? '';
    const explanation  = raw['explanation'] ?? raw['explain'] ?? raw['description'] ?? '';
    const example_url  = raw['example_url'] ?? raw['url'] ?? raw['tradingview_url'] ?? '';
    const notes        = raw['notes'] ?? raw['note'] ?? '';
    const submitted_by = raw['submitted_by'] ?? raw['by'] ?? '';

    const errors: string[] = [];
    if (!concept)     errors.push('concept is required');
    if (!explanation) errors.push('explanation is required');

    const entry: ConceptEntryInsert | null = errors.length === 0
      ? { concept, explanation, example_url: example_url || null, notes: notes || null, submitted_by: submitted_by || null }
      : null;
...
function downloadTemplate() {
  const csv = [
    ...
    '"Liquidity Sweep","A move engineered to trigger stop-losses beyond a key high or low before reversing in the true direction.",https://www.tradingview.com/x/abc123/,"Look for a wick beyond the level followed by a strong close back inside.",tradingjamiro49',
    '"Structural Break","When price closes beyond a significant swing high or low, confirming a change in market structure.",https://www.tradingview.com/x/xyz456/,"Requires a candle close — wicks alone do not count.",',
    '"Low Conviction Market","A market environment where price lacks directional momentum and oscillates within a tight range without clear bias.",,"Avoid taking setups during these conditions unless confluence is exceptional.",',
  ].join('\n');
  const blob = new Blob([csv], { type: 'text/csv' });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  a.href = url; a.download = 'concept_journal_template.csv'; a.click();
  URL.revokeObjectURL(url);
}

interface Props {
  onClose:    () => void;
  onImported: (entries: ConceptEntry[]) => void;
}

export default function ConceptsImporter({ onClose, onImported }: Props) {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const inserted = await bulkInsertConceptEntries(validRows.map(r => r.entry as ConceptEntryInsert));
      toast.success(`${inserted.length} concept${inserted.length !== 1 ? 's' : ''} imported.`);
      onImported(inserted);
      onClose();
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div style={{ background: '#131920', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 14, width: '100%', maxWidth: 760, maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <div>
            ...
            <div style={{ fontSize: '0.75rem', color: 'rgba(240,246,252,0.35)', marginTop: '0.15rem' }}>Bulk-import concept journal entries from a spreadsheet</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.25rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          >
            ...
            <span style={{ fontSize: '0.72rem', color: 'rgba(240,246,252,0.2)' }}>
              Columns: concept · explanation · example_url · notes · submitted_by
            </span>
          </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, background: '#131920' }}>
                      {['#', 'Status', 'Concept', 'Explanation', 'Example URL', 'Issues'].map((h, i) => (
                    ...
                    {rows.map(row => {
                      ...
                      return (
                        <tr key={row.index} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: ok ? 'transparent' : `${INVALID_RED}08` }}>
                          ...
                          <td style={{ ...tdStyle, fontSize: '0.8rem', color: '#f0f6fc', fontWeight: 500, maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {row.raw['concept'] || <span style={{ color: 'rgba(240,246,252,0.2)' }}>—</span>}
                          </td>
                          <td style={{ ...tdStyle, fontSize: '0.78rem', color: 'rgba(240,246,252,0.55)', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {row.raw['explanation'] || <span style={{ color: 'rgba(240,246,252,0.2)' }}>—</span>}
                          </td>
                          <td style={{ ...tdStyle, fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', color: ACCENT, maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {row.raw['example_url'] || row.raw['url'] || <span style={{ color: 'rgba(240,246,252,0.2)' }}>—</span>}
                          </td>
                          <td style={tdStyle}>
                            {row.errors.length > 0 && (
        ...
        <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <button
            ...
            style={{ padding: '0.65rem 1.5rem', background: (validRows.length === 0 || importing) ? 'rgba(0,200,255,0.1)' : 'linear-gradient(135deg, #00C8FF 0%, #0090b3 100%)', color: (validRows.length === 0 || importing) ? 'rgba(0,200,255,0.4)' : '#0A0A0F', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '0.875rem', cursor: (validRows.length === 0 || importing) ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', transition: 'all 0.15s' }}
          >
            {importing && <Loader2 size={14} style={{ animation: 'spin 0.8s linear infinite' }} />}
            {importing ? 'Importing...' : `Import ${validRows.length} concept${validRows.length !== 1 ? 's' : ''}`}
          </button>
